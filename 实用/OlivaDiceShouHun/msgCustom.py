...
代骰方式：在指令的末尾@目标用户''',
    ...
    'shst':'''【狩魂者跑团数据录入帮助文档】
1、玩家进群后，输入.st lock，该指令能锁定你的人物卡，在同一骰子的其他群切换人物卡时不影响本群。
（注意：如果是第一次用这个骰子创建人物卡，可在完成下一步的录入.st数据后再输入.st lock；如果不是第一次录入，就直接.st lock，并在录入数据后输入.st set [人物卡名]，切换到最新录入的人物卡）
2、玩家在填写完角色卡并审核通过后，将附表“信息总览表与检定公式”中的.st数据复制粘贴，输入群内，这样就完成了基础数据的录入。
3、玩家输入.st temp shouhun，使用狩魂者模板。
4、玩家输入.st blockrm 其它，删除多余数据。
5、玩家输入.sn，改变名片格式。
6、玩家输入.sn auto on，让名片数据能够自动变化。'''
}
